...
SENIOR_DEBT = 800_000_000                     # 선순위 대출 8억 (40%)
TOKEN_AMOUNT = 800_000_000                    # 토큰화 대상 8억 (40%)
INITIAL_PF_COUNT = 1                          # 초기 운영 PF 수
ROLLOVER_PF_COUNT = 1                         # 롤오버 시 편입 PF 수

# 토큰 단위 (소액 투자자 접근성)
TOKEN_UNIT_PRICE = 10_000                     # 토큰 1개당 가격 1만원
TOKENS_PER_PF = TOKEN_AMOUNT // TOKEN_UNIT_PRICE  # PF당 토큰 수 = 8억/1만 = 80,000개

# 수익 분배 구조
EQUITY_SHARE = 1 / 3                          # 지분수익 비율 (1/3)
TOKEN_SHARE = 2 / 3                           # 토큰수익 비율 (2/3)
# 순현금흐름 = 지분수익 + 토큰수익 (1:2 비율, 자기자본 4억 : 토큰 8억)
...
# FCF 비용구조
# 총비용 = 대출금 원금상환 + 선순위 이자비용 + OPEX + 사무위탁비 + 대수선비 + 세금
SENIOR_INTEREST_RATE = 0.06                   # 선순위 대출 연이자율 6%
OPEX_ANNUAL = 10_000_000                      # 연간 OPEX 1천만원 (1MW 기준)
ADMIN_FEE_ANNUAL = 10_000_000                 # 연간 사무위탁비 1,000만원
MAJOR_REPAIR_ANNUAL = 5_000_000               # 연간 대수선비 500만원
FIXED_COST_MONTHLY = (OPEX_ANNUAL + ADMIN_FEE_ANNUAL + MAJOR_REPAIR_ANNUAL) / 12
OPEX_MONTHLY = FIXED_COST_MONTHLY             # 월 고정비용 (OPEX + 사무위탁 + 대수선)
...
# α = 0.1 → CV 0.24 → 0.024, 피크/트러프 2.16 → 1.08
...
# 태양광 패널 열화 (Degradation)
DEGRADATION_RATE_ANNUAL = 0.005   # 연 0.5% 열화
DEGRADATION_RATE_MONTHLY = 1 - (1 - DEGRADATION_RATE_ANNUAL) ** (1/12)  # 월 환산
...
class SolarPFModel:
    ...
    def __init__(self, state_names, x_names, s_0, T, seed=42,
                 no_rollover=False, no_merge=False, p_complete_override=None):
        """
        매개변수
        ----------
        ...
        T           : int         — 에피소드 길이 (일)
        seed        : int         — 난수 시드
        no_rollover : bool        — True이면 롤오버(신규 PF 편입) 비활성화
        no_merge    : bool        — True이면 토큰 병합(Reverse Split) 비활성화
        p_complete_override : float — 초기완공확률 오버라이드 (None이면 P_COMPLETE_INIT 사용)
        """
        ...
        self.T = T
        self.no_rollover = no_rollover
        self.no_merge = no_merge
        self.p_complete_init = p_complete_override if p_complete_override is not None else P_COMPLETE_INIT
        self.seed = seed
    ...
    def _init_pf_list(self, count=INITIAL_PF_COUNT):
        ...
        for _ in range(count):
            pf = PFState(
                ...
                start_month=0,
                status='PRE',
                P_complete=self.p_complete_init,
            )
    ...
    def pv_engine(self, status, time_t, curtail_today=False):
        ...
        # 세전이익 (원금상환은 비용 불인정 → 세후 차감)
        taxable_income = monthly_revenue - OPEX_MONTHLY - monthly_interest
        if taxable_income > 0:
            after_tax = taxable_income * (1 - TAX_RATE)
        else:
            after_tax = taxable_income  # 손실 시 세금 없음
        monthly_fcf = after_tax - monthly_debt
    ...
    def _pv_engine_for_pf(self, pf: PFState, curtail_today: bool):
        ...
        # 열화 계수 (운영 경과 월수 기반)
        op_months = max(0, pf.local_time - T_CONSTRUCTION)
        degradation_factor = (1 - DEGRADATION_RATE_ANNUAL) ** (op_months / 12)

        monthly_revenue = price * avg_energy * 1000 * degradation_factor

        # 비용 계산
        if op_months < DEBT_REPAYMENT_YEARS * 12:
            monthly_interest = SENIOR_DEBT * (SENIOR_INTEREST_RATE / 12)
            monthly_debt = DEBT_MONTHLY_PAYMENT
        else:
            monthly_interest = 0.0
        ...
        # 열화 반영 감소 연금 (declining annuity)
        q = (1 - DEGRADATION_RATE_MONTHLY) / (1 + R_MONTHLY)
        annuity = q * (1 - q ** remaining_op) / (1 - q)
        annuity *= degradation_factor  # 현재 시점 열화도 반영
        discount = (1 + R_MONTHLY) ** (-months_to_start)
        pv = token_cf * annuity * discount
    ...
    def apply_events(self, t, current_nav_before_rollover=None):
        ...
        if result['success']:
            ...
            new_pf_nav = new_pf_value * self.p_complete_init
            new_total_nav = current_nav + new_pf_nav

            # 병합 기준가 설정 (T_MERGE_BASELINE 롤오버 직후)
            if not self.no_merge and t == T_MERGE_BASELINE and self.merge_baseline_price is None:
    ...
    def _estimate_new_pf_value(self):
        ...
        if taxable_income > 0:
            ...
            after_tax = taxable_income
        monthly_fcf = after_tax - DEBT_MONTHLY_PAYMENT
        token_cf = monthly_fcf * TOKEN_SHARE

        # PV 계산 (운영기간 전체, 열화 반영 감소 연금)
        q = (1 - DEGRADATION_RATE_MONTHLY) / (1 + R_MONTHLY)
        annuity = q * (1 - q ** T_OPERATION) / (1 - q)
    ...
    def _execute_token_issuance(self, current_nav, new_pf_value, Q):
        ...
        y = new_pf_value * self.p_complete_init
